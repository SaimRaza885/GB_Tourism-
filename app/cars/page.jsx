...
import { FaUsers, FaGasPump, FaCheckCircle, FaWhatsapp, FaArrowRight, FaRoad } from "react-icons/fa";
import Link from "next/link";
import PageBanner from "@/components/layout/PageBanner";
export default function CarsPage() {
    return (<main className="min-h-screen bg-background pb-20">
      <PageBanner height="50vh" badge="Mountain Ready" badgeIcon={FaRoad} title="Reliable Transport" image="https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?q=80&w=2070&auto=format&fit=crop" description="Navigate the rugged beauty of Gilgit-Baltistan with our specialized fleet of 4x4 vehicles and experienced local drivers."/>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-32">
        ...
        ].map((f, i) => (<div key={i} className="p-10 bg-card rounded-[2.5rem] border border-border group hover:border-primary/20 transition-all">
      ...
      {/* Car Listings */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-32">
        <SectionHeading badge="The Fleet" title={<>Choose Your <span className="text-primary italic">Ride</span></>} description="From heavy-duty SUVs for the Karakoram Highway to economical sedans for city travel."/>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          {cars.map((car) => (<div key={car.id} className="group bg-card rounded-[3rem] overflow-hidden border border-border shadow-sm hover:shadow-2xl transition-all duration-500">
