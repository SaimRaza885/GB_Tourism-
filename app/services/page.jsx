...
export default function ServicesPage() {
    ...
    return (<main className="min-h-screen bg-background pb-20">
            <PageBanner height="50vh" badge="Support Services" badgeIcon={FaMapMarkedAlt} title="Our Dedicated Services" image="https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?q=80&w=2070&auto=format&fit=crop" description="Comprehensive travel solutions tailored for the unique landscape of Gilgit-Baltistan."/>

            {/* Main Services Grid */}
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-32">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    {mainServices.map((s, i) => (<div key={i} className="group bg-card p-12 rounded-[3.5rem] border border-border hover:border-primary/20 transition-all duration-500 shadow-sm hover:shadow-2xl">
                            <div className="flex flex-col md:flex-row gap-8 items-start">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        {s.features.map((f, idx) => (<span key={idx} className="text-[10px] font-black uppercase tracking-widest bg-muted px-3 py-1.5 rounded-full text-foreground/70">
                                                {f}
                                            </span>))}
                                    </div>
            ...
            <section className="bg-foreground text-white py-32 rounded-[5rem] mx-4 sm:mx-8">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
                        {secondaryServices.map((s, i) => (<div key={i} className="flex gap-6 items-start">
