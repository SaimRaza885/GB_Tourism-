...
import { FaStar, FaMapMarkerAlt, FaHotel, FaSwimmer, FaWifi, FaParking, FaUtensils, FaAward } from "react-icons/fa";
...
export default function HotelsPage() {
    ...
    return (<main className="min-h-screen bg-background text-foreground/90">
            <PageBanner height="50vh" badge="Premium Accommodations" badgeIcon={FaHotel} title="Find Your Sanctuary" image="https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?q=80&w=2070&auto=format&fit=crop" description="Discover the most exclusive hotels and hidden gems across the majestic valleys of Gilgit-Baltistan."/>
            ...
            <section className="py-8 border-y border-border sticky top-[64px] lg:top-[80px] z-40 bg-background/80 backdrop-blur-xl">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex flex-col lg:flex-row gap-8 items-center justify-between">
                        <div className="flex flex-wrap items-center justify-center gap-3">
                            ...
                            {categories.map((cat) => (<button key={cat} onClick={() => setFilter(cat)} className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all border ${filter === cat
                ...
                : "bg-muted text-muted-foreground border-transparent hover:border-primary/30 hover:bg-background"}`}>
                        ...
                        <div className="flex items-center gap-6 bg-muted/50 p-1.5 rounded-2xl border border-border">
                            ...
                            <select value={locationFilter} onChange={(e) => setLocationFilter(e.target.value)} className="bg-background border border-border rounded-xl px-4 py-2 text-xs font-bold uppercase tracking-widest focus:ring-2 focus:ring-primary outline-none cursor-pointer">
            ...
            <section className="py-20">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <SectionHeading center badge="Your Stay" title={<>Premium <span className="text-primary italic">Accommodations</span></>} description={`Explore our collection of ${filteredHotels.length} hand-picked stays, each offering a unique window into the beauty of the north.`}/>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                        {filteredHotels.map((hotel) => (<Link key={hotel.id} href={`/hotels/${hotel.id}`} className="group bg-card rounded-4xl overflow-hidden border border-border shadow-sm hover:shadow-2xl hover:border-primary/20 transition-all duration-500 hover:-translate-y-2">
                                ...
                                <div className="p-6 space-y-4">
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        {hotel.amenities.slice(0, 3).map((amenity, idx) => (<div key={idx} className="flex items-center gap-1 text-[10px] text-muted-foreground bg-muted px-2 py-1 rounded-md">
                    ...
                    {filteredHotels.length === 0 && (<div className="text-center py-20 bg-muted rounded-[3rem] border-2 border-dashed border-border">
                            ...
                            <button onClick={() => { setFilter("All"); setLocationFilter("All"); }} className="mt-4 text-primary font-bold underline">
                                Clear all filters
                            </button>
                        </div>)}
                </div>
        ...
        ].map((item, i) => (<div key={i} className="bg-white/5 backdrop-blur-md border border-white/10 p-10 rounded-4xl group hover:bg-white/10 transition-all duration-500">
