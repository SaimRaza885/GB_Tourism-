...
import { FaStar, FaMapMarkerAlt, FaUtensils, FaCheckCircle, FaUserFriends, FaArrowLeft } from "react-icons/fa";
    ...
    return (<main className="min-h-screen bg-background pt-24 pb-20">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    <div className="lg:col-span-8">
                        <div className="aspect-video rounded-[2.5rem] overflow-hidden shadow-2xl">
                            <Image src={hotel.image} alt={hotel.name} className="w-full h-full object-cover" width={1200} height={800}/>
                        </div>
                    </div>
                    <div className="lg:col-span-4 grid grid-cols-1 gap-6">
                        {hotel.gallery?.slice(1, 3).map((img, idx) => (<div key={idx} className="aspect-16/10 rounded-3xl overflow-hidden shadow-xl">
                                <Image src={img} alt={`${hotel.name} gallery ${idx}`} className="w-full h-full object-cover hover:scale-110 transition-transform duration-700" width={1200} height={800}/>
                            </div>))}
                    </div>
                </div>
            </section>
                    ...
                    <div className="lg:col-span-2 space-y-10">
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                                {hotel.amenities.map((amenity, idx) => (<div key={idx} className="flex items-center gap-3 text-muted-foreground font-bold text-sm">
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="space-y-6">
                                {hotel.rooms ? hotel.rooms.map((room, idx) => (<div key={idx} className="p-8 bg-card rounded-4xl border border-border shadow-sm hover:shadow-xl transition-all duration-500 group">
                                    ...
                                    </div>)) : (<p className="text-muted-foreground italic">No specific rooms listed. Please contact for availability.</p>)}
                            </div>
                        </div>
                    </div>
            ...
            <section className="bg-muted/30 py-20">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <SectionHeading badge="Services" title={<>Added <span className="text-primary italic">Value</span></>} description="Inclusive services and professional assistance during your stay."/>
        ...
        ].map((s, i) => (<div key={i} className="bg-card p-10 rounded-[2.5rem] border border-border group hover:border-primary/20 transition-all">
