...
import { FaCalendarAlt, FaStar, FaMapMarkerAlt, FaQuestionCircle, FaShieldAlt, FaAward, FaUserTie } from "react-icons/fa";
import GalleryViewer from "@/components/tours/GalleryViewer";
...
export default async function TourDetailsPage({ params }) {
    ...
    return (<main className="bg-background min-h-screen pb-20">
            {/* Hero Section */}
            <div className="relative h-[70vh] w-full overflow-hidden group">
                <Image src={tour.image || "/placeholder.svg"} alt={tour.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" width={1200} height={800}/>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    <div className="lg:col-span-2 space-y-16">
                        ...
                        {/* Gallery */}
                        {tour.gallery && tour.gallery.length > 0 && (<section>
                        ...
                        {/* Video */}
                        {tour.videoUrl && (<section>
                                <h2 className="flex items-center gap-2 text-2xl font-bold text-foreground mb-6 border-l-4 border-primary pl-4">
                                    Video Tour
                                </h2>
                                <div className="rounded-2xl overflow-hidden shadow-lg aspect-video ring-1 ring-border">
                                    <iframe width="100%" height="100%" src={tour.videoUrl} title="Tour Video" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/>
                                </div>
                            </section>)}
                        ...
                        {tour.faqs && (<section>
                                ...
                                <div className="space-y-4">
                                    {tour.faqs.map((faq, idx) => (<div key={idx} className="bg-card rounded-xl border border-border p-6 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <section>
                            ...
                            {tour.reviews && tour.reviews.length > 0 ? (<div className="grid md:grid-cols-2 gap-6">
                                    {tour.reviews.map((review, idx) => (<div key={idx} className="bg-card p-6 rounded-2xl border border-border shadow-sm flex flex-col h-full">
                                            <div className="flex items-center justify-between mb-4">
                                                ...
                                                <div className="flex text-yellow-500">
                                                    {[...Array(5)].map((_, i) => (<FaStar key={i} className={i < review.rating ? "fill-current" : "text-gray-200"}/>))}
                                                </div>
                                            </div>
                                            <p className="text-muted-foreground italic grow">"{review.comment}"</p>
                                        </div>))}
                                </div>) : (<p className="text-muted-foreground p-4 bg-muted rounded-xl">No reviews yet for this destination. Be the first to review!</p>)}
                        </section>
                    </div>
                    ...
                    <div className="relative">
                        <div className="sticky top-24 space-y-8">
                            ...
                            <div className="bg-card rounded-3xl p-6 border border-border shadow-lg">
                                ...
                                <div className="space-y-4">
                                    {similarTours.map((st) => (<Link href={`/tours/${st.id}`} key={st.id} className="flex gap-3 group">
                                            <div className="w-20 h-16 rounded-lg overflow-hidden shrink-0 bg-muted">
                                                <Image src={st.image} alt={st.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" width={1200} height={800}/>
                                            </div>
