...
import { FaCamera, FaVideo, FaImages, FaWhatsapp, FaStar } from "react-icons/fa";
import SectionHeading from "@/components/shared/SectionHeading";
import Link from "next/link";
import PageBanner from "@/components/layout/PageBanner";
export default function PhotographyPage() {
    return (<main className="min-h-screen bg-background pb-20">
            <PageBanner height="50vh" badge="Capturing Memories" badgeIcon={FaCamera} title="Professional Photography" image="https://images.unsplash.com/photo-1454496522488-7a8e488e8606?q=80&w=2076&auto=format&fit=crop" description="Don't just see the mountains—keep them forever. Professional photography and videography services for your journey."/>

            {/* Services Grid */}
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-32">
                <SectionHeading center badge="Our Packages" title={<>Visual <span className="text-primary italic">Storytelling</span></>} description="Choose the perfect package to document your adventure in the Gilgit-Baltistan region."/>
            ...
            },
            {
                ...
                color: "bg-amber-500"
        ...
        ].map((pkg, i) => (<div key={i} className="bg-card rounded-[3rem] p-10 border border-border shadow-sm hover:shadow-2xl transition-all duration-500 group flex flex-col">
                            ...
                            <ul className="space-y-4 mb-10">
                                {pkg.features.map((f, idx) => (<li key={idx} className="flex items-center gap-3 text-xs font-bold text-foreground/70 uppercase tracking-widest">
            ...
            <section className="bg-foreground py-32 mb-32 overflow-hidden relative">
                <div className="absolute top-0 left-0 w-full h-full opacity-5 pointer-events-none">
                    <div className="grid grid-cols-4 gap-4 rotate-12 scale-150">
                        {[1, 2, 3, 4, 5, 6, 7, 8].map(i => (<div key={i} className="aspect-square bg-white rounded-3xl"/>))}
                    </div>
                </div>
        ...
        ].map((img, i) => (<div key={i} className={`aspect-3/4 rounded-4xl overflow-hidden group ${i % 2 === 1 ? 'translate-y-12' : ''}`}>
                                <Image src={img} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 brightness-75 group-hover:brightness-100" alt="Portfolio" width={1200} height={800}/>
                            </div>))}
                    </div>
                </div>
            </section>
