...
export default function DestinationsPage() {
    return (<main className="min-h-screen bg-background pb-32">
            <PageBanner height="50vh" badge="Our Regions" badgeIcon={FaGlobe} title="Discover The Valleys" image="https://images.unsplash.com/photo-1501785888041-af3ef285b470?q=80&w=2070&auto=format&fit=crop" description="From the ancient forts of Hunza to the cold desert of Skardu, explore the diverse landscapes of Gilgit-Baltistan."/>

            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                    {destinations.map((dest) => (<Link key={dest.id} href={`/destinations/${dest.id}`} className="group flex flex-col bg-card rounded-[3.5rem] overflow-hidden border border-border shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
                            <div className="aspect-ratio-[16/10] overflow-hidden relative">
                                <Image src={dest.image} alt={dest.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000" width={1200} height={800}/>
