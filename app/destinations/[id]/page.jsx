...
import { destinations, tours, hotels } from "@/data/index";
...
export default function DestinationDetailsPage({ params }) {
    ...
    return (<main className="min-h-screen bg-background pb-32">
            <PageBanner height="50vh" badge={`Explore ${destination.name}`} badgeIcon={FaGlobe} title={destination.name} image={destination.image} description={destination.description}/>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
                        <div className="lg:col-span-2 space-y-8">
                            ...
                            <div className="flex flex-wrap gap-3">
                                {destination.tags?.map((tag, i) => (<span key={i} className="px-5 py-2 bg-muted rounded-full text-[10px] font-black uppercase tracking-widest text-foreground/60 border border-border">
                                        # {tag}
                                    </span>))}
                            </div>
                        </div>
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32">
                <SectionHeading badge="Don't Miss" title={<>Key <span className="text-primary italic">Attractions</span></>} description="The must-visit spots that define the beauty and character of this region."/>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {destination.attractions?.map((attr, i) => (<div key={i} className="group bg-card p-10 rounded-[3rem] border border-border hover:border-primary/20 transition-all duration-500 shadow-sm hover:shadow-2xl">
            ...
            <section className="bg-muted/30 py-32 mb-32">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {relatedTours.slice(0, 3).map((tour) => (<Link key={tour.id} href={`/packages/${tour.id}`} className="group bg-card rounded-[2.5rem] overflow-hidden border border-border shadow-sm hover:shadow-2xl transition-all duration-500">
                                <div className="aspect-ratio-[16/10] overflow-hidden relative">
                                    <Image src={tour.image} alt={tour.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" width={1200} height={800}/>
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {relatedHotels.slice(0, 2).map((hotel) => (<Link key={hotel.id} href={`/hotels/${hotel.id}`} className="group relative h-[400px] rounded-[3.5rem] overflow-hidden shadow-xl">
                            <Image src={hotel.image} alt={hotel.name} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" width={1200} height={800}/>
