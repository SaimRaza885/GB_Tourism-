...
import { FaCalendarAlt, FaStar, FaMapMarkerAlt, FaCheckCircle, FaQuestionCircle, FaShieldAlt, FaPlaneDeparture, FaSuitcase, FaUserShield } from "react-icons/fa";
...
export default async function PackageDetailsPage({ params }) {
    ...
    return (<main className="bg-background min-h-screen pb-20">
            ...
            <div className="relative h-[70vh] w-full overflow-hidden group">
                <Image src={pkg.image || "/placeholder.svg"} alt={pkg.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" width={1200} height={800}/>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    <div className="lg:col-span-2 space-y-16">
                        ...
                        {pkg.includes && (<section className="bg-gradient-to-br from-card to-muted p-8 rounded-3xl border border-border shadow-sm">
                                <h2 className="text-2xl font-bold text-foreground mb-6 flex items-center gap-2">
                                    <FaShieldAlt className="text-primary"/> What's Included
                                </h2>
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                                    {pkg.includes.map((item, idx) => (<div key={idx} className="flex items-center gap-3">
                        ...
                        {pkg.itinerary && (<section>
                                ...
                                <div className="border-l-2 border-primary/30 ml-3 space-y-8">
                                    {pkg.itinerary.map((day, idx) => (<div key={idx} className="relative pl-8 group">
                        ...
                        {/* Gallery */}
                        {pkg.gallery && pkg.gallery.length > 0 && (<section>
                        ...
                        {pkg.videoUrl && (<section>
                                ...
                                <div className="rounded-2xl overflow-hidden shadow-lg aspect-video ring-1 ring-border">
                                    <iframe width="100%" height="100%" src={pkg.videoUrl} title="Package Video" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/>
                                </div>
                            </section>)}
                        ...
                        {pkg.faqs && (<section>
                                ...
                                <div className="space-y-4">
                                    {pkg.faqs.map((faq, idx) => (<div key={idx} className="bg-card rounded-xl border border-border p-6 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="relative">
                        <div className="sticky top-24 space-y-8">
                            ...
                            <div className="bg-card rounded-3xl p-6 border border-border shadow-lg">
                                ...
                                <div className="space-y-4">
                                    {similarPackages.map((sp) => (<Link href={`/packages/${sp.id}`} key={sp.id} className="flex gap-3 group">
                                            <div className="w-20 h-16 rounded-lg overflow-hidden shrink-0 bg-muted">
                                                <Image src={sp.image} alt={sp.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" width={1200} height={800}/>
                                            </div>
