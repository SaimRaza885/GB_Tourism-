...
export default function GuideDetailsPage({ params }) {
    ...
    return (<main className="min-h-screen bg-background pt-24 pb-20">
                    ...
                    <div className="lg:col-span-5">
                        <div className="relative">
                            <div className="aspect-square rounded-[3rem] overflow-hidden shadow-2xl skew-y-1 hover:skew-y-0 transition-transform duration-700">
                                <Image src={guide.image} alt={guide.name} className="w-full h-full object-cover" width={1200} height={800}/>
                            </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-8 py-8 border-y border-border/50">
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2 mt-1">
                                    {guide.languages.map((l, i) => (<span key={i} className="text-xs font-bold bg-muted px-2 py-0.5 rounded-md">{l}</span>))}
                                </div>
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                        <div className="bg-card p-12 rounded-[3.5rem] border border-border shadow-sm">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                {guide.skills.map((skill, i) => (<div key={i} className="flex items-center gap-3 group">
                        ...
                        <div className="bg-card p-12 rounded-[3.5rem] border border-border shadow-sm">
                            ...
                            <div className="flex flex-wrap gap-4">
                                {guide.areasCovered.map((area, i) => (<div key={i} className="px-6 py-3 bg-muted rounded-2xl flex items-center gap-3 group hover:bg-primary/10 transition-colors">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-16">
                        <div className="max-w-2xl">
                            <SectionHeading badge="Adventure Gallery" title={<>Captured by <span className="text-primary italic">{guide.name.split(' ')[0]}</span></>} description="A glimpse into the expeditions and moments shared with travelers across the mountains."/>
                        </div>
                        <div className="flex gap-4 mb-4">
                            <button onClick={scrollPrev} className="w-14 h-14 rounded-full border border-border flex items-center justify-center hover:bg-primary hover:text-white transition-all shadow-sm active:scale-90">
                                <FaChevronLeft />
                            </button>
                            <button onClick={scrollNext} className="w-14 h-14 rounded-full border border-border flex items-center justify-center hover:bg-primary hover:text-white transition-all shadow-sm active:scale-90">
                                <FaChevronRight />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="embla" ref={emblaRef}>
                        <div className="embla__container flex gap-6">
                            {guide.gallery?.map((img, i) => (<div key={i} className="embla__slide flex-[0_0_100%] sm:flex-[0_0_80%] lg:flex-[0_0_60%] min-w-0">
                                    <div className="relative h-[500px] rounded-[3rem] overflow-hidden shadow-2xl group cursor-grab active:cursor-grabbing">
                                        <Image src={img} alt={`Adventure with ${guide.name} ${i}`} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" width={1200} height={800}/>
                    ...
                    <div className="flex justify-center gap-2 mt-12">
                        {guide.gallery?.map((_, i) => (<button key={i} onClick={() => emblaApi && emblaApi.scrollTo(i)} className={`h-1.5 rounded-full transition-all duration-300 ${selectedIndex === i ? 'w-12 bg-primary' : 'w-2 bg-border'}`}/>))}
                    </div>
                </div>
            </section>
                ...
                <div className="max-w-5xl mx-auto px-4 text-center">
                    ...
                    <div className="flex items-center justify-center gap-4">
                        <div className="w-12 h-12 rounded-full overflow-hidden bg-muted">
                            <Image src="https://i.pravatar.cc/150?u=jane" alt="User" width={1200} height={800}/>
                        </div>
