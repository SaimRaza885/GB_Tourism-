...
import { stats, services } from "@/data/index";
...
export default function TrustSection() {
    return (<section className="py-20 lg:py-28 bg-background overflow-hidden relative">
            {/* Decorative patterns */}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
                <SectionHeading center badge="Trusted by Travelers" title={<>Why Travelers Choose <span className="text-primary italic">Gilgit Tourism</span></>} description="We combine local expertise with world-class service standards to ensure your journey through the Karakoram is safe, seamless, and unforgettable."/>

                {/* Benefits Grid */}
                <div className="grid md:grid-cols-3 gap-8 lg:gap-12 mb-20">
                    {services.map((service) => {
            const Icon = iconMap[service.icon];
            return (<div key={service.id} className="group p-8 rounded-3xl bg-card border border-border shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300">
                        ...
                        <div className="lg:col-span-2 bg-white/5 backdrop-blur-md rounded-3xl p-8 border border-white/10">
                            ...
                            <div className="flex flex-wrap gap-4">
                                <div className="flex -space-x-3">
                                    {[11, 12, 13].map((i) => (<Image key={i} src={`https://i.pravatar.cc/100?u=${i}`} alt="user" className="w-10 h-10 rounded-full border-2 border-foreground" width={1200} height={800}/>))}
                                </div>
