...
import { FaStar, FaMapMarkerAlt, FaSwimmer, FaWifi, FaParking, FaUtensils } from "react-icons/fa";
function HotelSection() {
    let top_hotels = hotels.slice(0, 3);
    return (<section className="py-20">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <SectionHeading center badge="Your Stay" title={<>Premium <span className="text-primary italic">Accommodations</span></>} description={`Explore our collection of ${hotels.length} hand-picked stays, each offering a unique window into the beauty of the north.`}/>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                    {top_hotels.map((hotel) => (<Link key={hotel.id} href={`/hotels/${hotel.id}`} className="group bg-card rounded-4xl overflow-hidden border border-border shadow-sm hover:shadow-2xl hover:border-primary/20 transition-all duration-500 hover:-translate-y-2">
                ...
                {top_hotels.length === 0 && (<div className="text-center py-20 bg-muted rounded-[3rem] border-2 border-dashed border-border">
