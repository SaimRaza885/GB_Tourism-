...
export default function DestinationsSection() {
    return (<section className="py-24 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeading badge="Where to go" title={<>Major <span className="text-primary italic">Regions</span> to Explore</>} description="Gilgit-Baltistan is a vast land of diverse cultures and landscapes. Choose your region and start adventuring."/>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {destinations.map((dest) => (<Link key={dest.id} href={`/destinations/${dest.id}`} className="group relative h-[350px] rounded-[2.5rem] overflow-hidden shadow-xl">
              {/* Background Image */}
