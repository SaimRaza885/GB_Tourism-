...
export default function PackagesSection() {
    ...
    return (<section className="py-24 bg-muted/30">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <SectionHeading badge="All-Inclusive" title={<>Featured Tour <span className="text-primary">Packages</span></>} linkHref="/packages" linkText="View All Packages"/>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {featuredPackages.map((pkg) => (<Link key={pkg.id} href={`/packages/${pkg.id}`} className="group">
                            <div className="bg-card rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 h-full flex flex-col">
                                ...
                                <div className="p-5 flex flex-col grow">
                                    ...
                                    <div className="space-y-2 mb-4">
                                        {pkg.includes?.slice(0, 2).map((item, idx) => (<div key={idx} className="flex items-center gap-2 text-xs text-muted-foreground">
