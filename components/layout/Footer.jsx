          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-end gap-6">
              ...
              <div className="flex -space-x-4">
                {[1, 2, 3, 4].map((i) => (<img key={i} src={`https://i.pravatar.cc/100?u=${i + 10}`} alt="User" className="w-12 h-12 rounded-full border-4 border-primary ring-2 ring-white/10"/>))}
...
function LeadGenerationButton() {
    ...
    return (<>
      <button onClick={() => setIsOpen(true)} className="relative px-8 py-4 bg-secondary text-secondary-foreground rounded-full font-bold text-lg hover:bg-secondary/90 transition-all hover:scale-105 shadow-xl whitespace-nowrap">
        Request a Quote
      </button>

      <BookingModal isOpen={isOpen} onClose={() => setIsOpen(false)} item={item} type="plan"/>
    </>);
}
