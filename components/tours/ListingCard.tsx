...
import { ListingType, ListingItem, ListingCardProps } from "@/types";


export default function ListingCard({ item, type = "tour" }: ListingCardProps) {

  const [isModalOpen, setIsModalOpen] = useState(false)

  const whatsappLink =
    `https://wa.me/${item.whatsapp?.replace(/[^0-9]/g, "")}?text=Hi, I'm interested in ${item.name}`
    ...
    <div className="group bg-card rounded-2xl overflow-hidden shadow-sm border border-border hover:shadow-xl transition-all duration-300 hover:-translate-y-1 flex flex-col h-full">
      ...
      <div className="p-5 pt-0 mt-auto space-y-2">
        <Button
          ...
          className="w-full py-6 bg-primary text-primary-foreground rounded-xl font-bold"
        >
        ...
        <a
          ...
          className="flex items-center justify-center gap-2 w-full py-3 bg-muted/50 text-muted-foreground rounded-xl font-medium hover:bg-muted transition-colors text-sm"
        >
